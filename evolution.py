...
def crossover(x, y):
    ...
    # find a random crossover point not at the ends of the shorter chromosome
    crossover_point = randint(1, len(shorter.chromosome) - 1)
...
def mutate(individual, genes, mutation_probability, structure_mutation_probability, num_mutations):
    ...
    for location in locations_to_modify:
        # if this is here, mutation_probability is the probability of each mutation -- do we want this?
        if random() <= mutation_probability:
            # randomly change the gene at the location based on the given range of values
            individual.chromosome[location] = choice(genes)
...
def trim_population(population, fitness_list, pruning_num):
    ...
    sorted_population, sorted_fitness_list = zip(
        *sorted(
